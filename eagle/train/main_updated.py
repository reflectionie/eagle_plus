...
@torch.no_grad()
def getkacc(model, data, head, max_length=5, input_prob = False, input_logits = False):
    assert (input_prob is True and input_logits is True), "input_prob and input logits can't be Ture simultaneously"
...
for epoch in range(num_epochs + 1):
    ...
    # if True:
        ...
        for batch_idx, data in enumerate(tqdm(test_loader)):
            with torch.no_grad():
                if batch_idx < 10:
                    acces = getkacc(model, data, head, max_length=5, input_prob=config.input_prob, input_logits=config.input_logits)
